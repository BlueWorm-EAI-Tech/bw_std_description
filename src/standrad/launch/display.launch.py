...
def launch_robot(context, package_share):
    ...
    model = model_override or f"{package_share}/urdf/standard_{variant}.urdf"
...
def generate_launch_description():
    package_share = get_package_share_directory("standard")
    ...
    return LaunchDescription(
        [
            DeclareLaunchArgument(
                ...
                description=(
                    "Robot form: v1 (full), v2 (arms), v3 (standard_patrol), "
                    "v4 (standard_arm), v5 (single arm with base plate), "
                    "v6 (standard_arm chassis)"
                ),
            ),
            DeclareLaunchArgument(
                "model",
            ...
            DeclareLaunchArgument(
                "rviz_config",
                default_value=f"{package_share}/rviz/standard.rviz",
